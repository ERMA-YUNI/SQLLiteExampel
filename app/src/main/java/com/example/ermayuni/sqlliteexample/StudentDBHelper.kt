...
class StudentDBHelper(context: Context) : SQLiteOpenHelper(
    ...
    fun searchStudentByNIM(nim: String): ArrayList<StudentModel> {
        ...
        val cursor: Cursor? = try {
            db.rawQuery(
                "SELECT * FROM ${DBContract.UserEntry.TABLE_NAME} WHERE ${DBContract.UserEntry.COLUMN_NIM} = ?", arrayOf(nim)
            )
    ...
    fun searchStudentByName(name: String): ArrayList<StudentModel> {
        ...
        val cursor: Cursor? = try {
            db.rawQuery(
                "SELECT * FROM ${DBContract.UserEntry.TABLE_NAME} WHERE ${DBContract.UserEntry.COLUMN_NAME} LIKE ?", arrayOf("%$name%")
            )
        } catch (e: SQLiteException) {
            db.execSQL(SQL_CREATE_ENTRIES)
            return ArrayList()
